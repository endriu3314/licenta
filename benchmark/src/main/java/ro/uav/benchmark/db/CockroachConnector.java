...
public final class CockroachConnector implements DatabaseConnector {
    private final DatabaseProfile profile;

    @Override
    public Connection connect() throws SQLException {
        log.debug("Connecting to CockroachDB at {}:{}", profile.host(), profile.port());
    ...
    public void clearCache(Connection connection) throws SQLException {
        try (var stmt = connection.createStatement()) {
            // CockroachDB doesn't expose direct cache clearing
            // Close and reopen connection is the best strategy
            log.debug("CockroachDB — connection-based cache strategy");
        }
    }

    @Override
    public AppConfig.DatabaseType type() {
        return AppConfig.DatabaseType.COCKROACH;
    }

    @Override
    public String jdbcUrl() {
        return "jdbc:postgresql://%s:%d/%s?sslmode=disable"
